...
parser.add_argument('--random_blocks', action='store_true', help='Perform random block substitution')
parser.add_argument('--param_target', default=0.5, type=float, help='How many parameters to sub')
parser.add_argument('--moonshine', action='store_true',help='Use if you want uniform block sub, like in the original paper')
parser.add_argument('--sanity', action='store_true', help='Run ablation tests.')
parser.add_argument('--n_trials', default=100, help='Number of hill climbing restarts to do')
parser.add_argument('--drop_until', default=1, type=int, help='Where to start from in the mega dataframe for random training')
...
def random_conv_sub(net,param_target):
    ...
    while((get_no_params(net) > param_target) or (get_no_params(net) < (param_target-(param_target*0.05)))):
        convs = []
        for m in net.modules():
            conv = choice(conv_options)
            convs.append(conv)

        net   = WideResNet(args.wrn_depth, args.wrn_width, Conv, MaskBlock,
                    num_classes=num_classes, dropRate=0,
                    s=args.AT_split, convs=convs)
    return convs
...
if __name__ == '__main__':
    ...
    def load_network(loc, masked=False):
        ...
        net = WideResNet(args.wrn_depth, args.wrn_width, SavedConv, SavedBlock, num_classes=num_classes, dropRate=0,masked=masked)
    ...
    if args.mode == 'teacher':
        ...
        if args.resume:
            ...
            teach = WideResNet(args.wrn_depth, args.wrn_width, Conv, Block, num_classes=num_classes, dropRate=0)
        ...
        teach, _ = load_network('checkpoints/%s.t7' % args.teacher_checkpoint, masked=True)
        teach_optim = optim.SGD(teach.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.weightDecay)

        convs = []
        if args.moonshine:
            print("Crowley's moonlit knowledge approaches")
            convs = [Conv for m in teach.modules()]

            depth = args.wrn_depth
            width = args.wrn_width

            if args.diff_shape:
                depth = args.stu_depth
                width = args.stu_width

            student = WideResNet(depth, width, Conv, BasicBlock, num_classes=num_classes, dropRate=0, masked=True, convs=convs)

        elif len(args.from_genotype) > 0:
            row = pd.read_csv(args.from_genotype)
            ...
            student = WideResNet(args.wrn_depth, args.wrn_width, Conv, BasicBlock, num_classes=num_classes, dropRate=0,masked=True)
            for i, c in enumerate(convs):
                student = update_block(i, student, string_to_conv[c])
